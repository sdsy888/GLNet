...
class fpn_module_global(nn.Module):
    ...
    def forward(self,
        ...
        if ps2_ext is None:
            ...
            p = self._concatenate(torch.cat((p5, ps2_ext[0]), dim=1), torch.cat((p4, ps2_ext[1]), dim=1),
                                  torch.cat((p3, ps2_ext[2]), dim=1), torch.cat((p2, ps2_ext[3]), dim=1))
            ps3 = self.smooth(p)
            output = self.classify(ps3)
...
class fpn_module_local(nn.Module):
    ...
    def forward(self, c2, c3, c4, c5, c2_ext, c3_ext, c4_ext, c5_ext, ps0_ext, ps1_ext, ps2_ext):
        ...
        p4 = self._upsample_add(
            p5,
            self.latlayer1(torch.cat([c4] + [F.interpolate(c4_ext[0], size=c4.size()[2:], **self._up_kwargs)], dim=1)))
        p3 = self._upsample_add(
            p4,
            self.latlayer2(torch.cat([c3] + [F.interpolate(c3_ext[0], size=c3.size()[2:], **self._up_kwargs)], dim=1)))
        p2 = self._upsample_add(
            p3,
            self.latlayer3(torch.cat([c2] + [F.interpolate(c2_ext[0], size=c2.size()[2:], **self._up_kwargs)], dim=1)))
        ps0 = [p5, p4, p3, p2]
...
class fpn(nn.Module):
    ...
    def _sample_grid(self, fm, bbox, sampleSize):
        ...
        gridMap = np.array([[(cnt_w / (sampleSize[1] - 1), cnt_h / (sampleSize[0] - 1))
                             for cnt_w in range(sampleSize[1])] for cnt_h in range(sampleSize[0])])
    ...
    def collect_local_fm(self,
        ...
        with torch.no_grad():
            ...
            if self.patch_n == 0:
                ...
                self.ps3_l.append(torch.cat(self.ps3_b, dim=0))
                # a list of tensors
        ...
        if mode == 1:
            ...
            with torch.no_grad():
                if self.patch_n == 0:
                    ...
                    self.output_g, self.ps0_g, self.ps1_g, self.ps2_g, self.ps3_g = self.fpn_global.forward(
                        self.c2_g, self.c3_g, self.c4_g, self.c5_g)
            ...
            # tt = [self._crop_global(f, top_lefts, ratio) for f in self.ps1_g]
            # for i in tt:
            #     for ii in i:
            #         print(ii.shape)
            # import pdb
            # print('check local dim:', c5_l.shape)
            # print('check dim---')
            # pdb.set_trace()
            # global's 1x patch cat
            output_l, ps0_l, ps1_l, ps2_l, ps3_l = self.fpn_local.forward(
                c2_l, c3_l, c4_l, c5_l, self._crop_global(self.c2_g, top_lefts, ratio),
                self._crop_global(self.c3_g, top_lefts, ratio), self._crop_global(self.c4_g, top_lefts, ratio),
                self._crop_global(self.c5_g, top_lefts,
                                  ratio), [self._crop_global(f, top_lefts, ratio) for f in self.ps0_g],
            ...
            output_g, ps0_g, ps1_g, ps2_g, ps3_g = self.fpn_global.forward(c2_g,
                                                                           c3_g,
                                                                           c4_g,
                                                                           c5_g,
                                                                           c2_ext=self.c2_l,
